...
import { MEMBERS_LINK, BASE_URL, AWARDS_LINK } from '../globals/links'
...
export const Awards = (props) => {
    ...
    let fetchHistory = async () => {
        ...
        let link;
        user.role.name == "Admin" ? link = `${AWARDS_LINK}?offset=${offset}&date=${date}&userId&status` : link = `${AWARDS_LINK}?offset=${offset}&date=${date}&userId=${user.user_id}&status`
        console.log(link)
                                    ...
                                    <tbody>
                                        {history.map((request, i) => (
                                            <tr key={i}>
                                                ...
                                                {user.role.name == "Admin" ? (
                                                    <td>
                                                        <ButtonStyle disabled={request.status !== "Pending" ? true : false} className="btn btn-success" id={request.award_id} name="Approve" onClick={doApprove}>{declineLoading ? 'Loading...' : 'Approve'}</ButtonStyle>
                                                    </td>
                                                ) : (null)}
                                            </tr>
                                        ))}
